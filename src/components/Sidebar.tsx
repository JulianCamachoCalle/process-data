...
export function Sidebar({
  ...
  const renderSheetLinks = (groupSheets: string[]) => (
  ...
  const renderDropdownSection = ({
    ...
    <div className={`space-y-1.5 ${withTopPadding ? 'pt-2' : ''}`}>
      <button
        type="button"
        onClick={onToggle}
        className={`group w-full px-4 pt-3 pb-2 inline-flex items-center justify-between text-[11px] font-semibold text-gray-400 uppercase tracking-[0.18em] ${collapsed ? 'md:justify-center md:px-2' : ''}`}
        title={collapsed ? title : undefined}
      >
        <span className={`inline-flex items-center gap-2 ${collapsed ? 'md:hidden' : ''}`}>
          {icon}
          <span>{title}</span>
        </span>
        {!collapsed && (
      ...
      <div className={`overflow-hidden transition-all duration-200 ease-out ${isOpen ? 'max-h-[1400px] opacity-100' : 'max-h-0 opacity-0'}`}>
    ...
    <aside className={`app-sidebar ${className}`}>
      ...
      <nav className="min-h-0 flex-1 overflow-y-auto px-3 py-3 sm:px-4 sm:py-4">
        <Link
          ...
          className={`group flex items-center w-full text-left px-4 py-3 rounded-xl border transition-all duration-200 mb-3 ${location.pathname === '/dashboard'
        ...
        {isAdmin && groupedSheets.base.length > 0 && renderDropdownSection({
          title: 'Tablas base',
          icon: <Layers3 size={12} className="text-red-500" />,
          isOpen: isBaseOpen,
          onToggle: () => setIsBaseOpen((prev) => !prev),
          children: renderSheetLinks(groupedSheets.base),
        })}

        {isAdmin && renderDropdownSection({
          title: 'Tablas operativas',
          icon: <Workflow size={12} className="text-red-500" />,
          isOpen: isOperativasOpen,
          onToggle: () => setIsOperativasOpen((prev) => !prev),
          withTopPadding: true,
          children: (
            <>
              {renderSheetLinks(groupedSheets.operativas)}
              <Link
                to="/operativas/estadisticas-vendedor"
                onClick={() => onNavigate?.()}
                title={collapsed ? 'Estadísticas de vendedor' : undefined}
                className={`group flex items-center w-full text-left px-4 py-2.5 rounded-xl border transition-all duration-200 ${isEstadisticasVendedorActive
                    ? 'bg-white/10 text-white font-semibold border-red-500/70 shadow-[0_8px_24px_-18px_rgba(255,255,255,0.8)]'
                    : 'text-gray-400 border-transparent hover:bg-white/5 hover:text-gray-100 hover:border-white/10'
                  } ${collapsed ? 'md:justify-center md:px-2' : 'gap-3'}`}
              >
              ...
              <Link
                ...
                className={`group flex items-center w-full text-left px-4 py-2.5 rounded-xl border transition-all duration-200 ${isResumenVendedorActive
                    ? 'bg-white/10 text-white font-semibold border-red-500/70 shadow-[0_8px_24px_-18px_rgba(255,255,255,0.8)]'
                    : 'text-gray-400 border-transparent hover:bg-white/5 hover:text-gray-100 hover:border-white/10'
                  } ${collapsed ? 'md:justify-center md:px-2' : 'gap-3'}`}
              >
        ...
        {renderDropdownSection({
          title: 'Meta Ads',
          icon: <Megaphone size={12} className="text-red-500" />,
          isOpen: isMetaAdsOpen,
          onToggle: () => setIsMetaAdsOpen((prev) => !prev),
          withTopPadding: true,
          children: (
            <>
              <Link
        ...
        {renderDropdownSection({
          title: 'Meta Pages',
          icon: <BookImage size={12} className="text-red-500" />,
          isOpen: isMetaPagesOpen,
          onToggle: () => setIsMetaPagesOpen((prev) => !prev),
          withTopPadding: true,
          children: (
            <Link
        ...
        {renderDropdownSection({
          title: 'Kommo CRM',
          icon: <Database size={12} className="text-red-500" />,
          isOpen: isKommoOpen,
          onToggle: () => setIsKommoOpen((prev) => !prev),
          withTopPadding: true,
          children: (
            <>
              {isAdmin && (
        ...
        {isAdmin && renderDropdownSection({
          title: 'Exportaciones',
          icon: <Send size={12} className="text-red-500" />,
          isOpen: isExportOpen,
          onToggle: () => setIsExportOpen((prev) => !prev),
          withTopPadding: true,
          children: (
            <Link
        ...
        {isAdmin && renderDropdownSection({
          title: 'Acceso',
          icon: <UserCog size={12} className="text-red-500" />,
          isOpen: isAccessOpen,
          onToggle: () => setIsAccessOpen((prev) => !prev),
          withTopPadding: true,
          children: (
            <Link
              to="/admin/users"
